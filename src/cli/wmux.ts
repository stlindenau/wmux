...
async function cmdServeApi(args: string[]): Promise<void> {
  ...
  const host = getFlag(args, '--host') || '127.0.0.1';
  const serverDir = (process as any).resourcesPath
    ? path.join((process as any).resourcesPath, 'wmux-orchestrator', 'server')
    : path.resolve(__dirname, '../../resources/wmux-orchestrator/server');

  const child = spawn('python3', ['-m', 'uvicorn', 'app:app', '--host', host, '--port', port], {
    cwd: serverDir,
    stdio: 'inherit',

...
function remoteTarget(): { host: string; port: number } | null {
...
function connectTransport(onConnect: () => void): net.Socket {
  ...
  return remote
    ? net.connect({ host: remote.host, port: remote.port }, onConnect)
    : net.connect({ path: pipePath }, onConnect);
}
...
function sendHook(): void {
  ...
  // whole message).
  params.seq = Math.round((performance.timeOrigin + performance.now()) * 1000);

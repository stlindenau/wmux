...
.EXAMPLE
powershell -ExecutionPolicy Bypass build-and-test-bridge-fixed.ps1
#>
...
try {
    ...
    if (-not $SkipBuild) {
        ...
        if ($cscPath -like "*dotnet.exe") {
            # Create temporary project for .NET CLI
...
"@
            ...
            try {
                ...
                & $cscPath build -c Release -o $scriptDir
                Pop-Location
            } finally {
                Remove-Item -Recurse -Force $tempDir -ErrorAction SilentlyContinue
            }
        } else {
            # Use .NET Framework csc directly
    ...
    $pipeTestCmd = 'echo "pipe test" | powershell -c "`$p = New-Object System.IO.Pipes.NamedPipeClientStream(''.'', ''wmux-bridge-poc''); `$p.Connect(); `$w = New-Object System.IO.StreamWriter(`$p); `$w.WriteLine(''test''); `$w.Flush(); `$p.Close()"'
    Write-Host "   $pipeTestCmd" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  • Install .NET Framework 4.0+ or .NET 6+" -ForegroundColor White
    Write-Host "  • Check if antivirus is blocking compilation" -ForegroundColor White
    exit 1
}
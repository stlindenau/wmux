...
/*
 * Windows AF_HYPERV to Named Pipe Bridge - Standalone PoC
 *
 * This bridge demonstrates secure VSOCK communication from WSL2 to Windows
 * via AF_HYPERV sockets, relaying to a named pipe for integration with wmux.
 *
 * Architecture:
 * WSL2 (AF_VSOCK) ↔ Windows Bridge (AF_HYPERV ↔ Named Pipe) ↔ wmux pipe
 *
 * Compile: csc /out:wmux-hyperv-bridge.exe windows-hyperv-bridge.cs
...
namespace WmuxHypervBridge
{
    class HypervBridge
    {
        ...
        private const string WMUX_PIPE_NAME = "wmux-bridge-poc"; // PoC pipe, not the real wmux pipe
        ...
        {
            Console.WriteLine("🔗 wmux AF_HYPERV to Named Pipe Bridge - PoC");
            Console.WriteLine("=============================================");
            Console.WriteLine();
        ...
        public async Task StartAsync()
        {
            ...
            Console.WriteLine($"   echo \"pipe test\" | powershell -c \"$p = New-Object System.IO.Pipes.NamedPipeClientStream('.', '{WMUX_PIPE_NAME}'); $p.Connect(); $w = New-Object System.IO.StreamWriter($p); $w.WriteLine('test'); $w.Flush(); $p.Close()\"");
            Console.WriteLine();
            ...
            try
            {
                await Task.WhenAll(hypervTask, namedPipeTask);
            }
        ...
        private bool RegisterHypervService()
        {
            try
            {
                var servicePath = $@"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Virtualization\GuestCommunicationServices\{WMUX_SERVICE_GUID}";
        ...
        private async Task StartHypervServerAsync()
        {
            try
            {
                ...
                // Real AF_HYPERV would require P/Invoke to Windows socket APIs
                Console.WriteLine("⚠️  AF_HYPERV server: Using TCP simulation (real implementation needs native code)");
                ...
                while (!_cancellationTokenSource.Token.IsCancellationRequested)
                {
                    ...
                    if (tcpClient != null)
                    {
                        _ = Task.Run(() => HandleHypervClientAsync(tcpClient), _cancellationTokenSource.Token);
                    }
                }
            ...
            try
            {
                using (hypervClient)
                {
                    ...
                    while (hypervClient.Connected && !_cancellationTokenSource.Token.IsCancellationRequested)
                    {
                        ...
                        var response = $"{{\"status\":\"relayed\",\"client\":\"{clientId}\",\"timestamp\":\"{DateTime.UtcNow:yyyy-MM-ddTHH:mm:ss.fffZ}\"}}";
        ...
        private async Task RelayToNamedPipeAsync(string message, string clientId)
        {
            try
            {
                using (var pipeClient = new NamedPipeClientStream(".", WMUX_PIPE_NAME, PipeDirection.InOut))
                {
                    ...
                    {
                        var relayMessage = $"[HYPERV:{clientId}] {message}";
            ...
            {
                Console.WriteLine($"⚠️  Named pipe connection timeout - no wmux server running?");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                while (!_cancellationTokenSource.Token.IsCancellationRequested)
                {
                        ...
                        PipeOptions.Asynchronous))
                    {
                        ...
                        _ = Task.Run(() => HandleNamedPipeClientAsync(pipeServer), _cancellationTokenSource.Token);
                    }
                }
            }
            catch (OperationCanceledException)
